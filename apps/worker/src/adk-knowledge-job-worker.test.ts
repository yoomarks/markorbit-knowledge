...
describe("ADK knowledge job worker", () => {
  ...
  it(
    "quarantines persistence uncertainty after provider success instead of replaying provider work",
    async () => {
      const store = new MemoryAiKnowledgeJobStore();
      enqueue(store);
      let providerCalls = 0;
  ...
  it(
    "recovers only explicitly safe queue states and quarantines stale running work",
    () => {
      const store = new MemoryAiKnowledgeJobStore();
      ...
      expect(store.get(staleRunning.id)?.error).toBe(
