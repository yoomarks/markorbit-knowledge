...
import { runRepresentativeSourceActivationWave } from "./representative-source-activation-runner";
...
async function main(): Promise<void> {
  ...
  const result = await runRepresentativeSourceActivationWave({
    baseUrl,
    workspaceId,
    apply,
  ...
  if (apply && result.summary.failed > 0) process.exitCode = 1;
}
