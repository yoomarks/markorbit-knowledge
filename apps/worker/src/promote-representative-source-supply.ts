...
import { runRepresentativeSupplyPromotionWave } from "./representative-supply-promotion";
...
async function main(): Promise<void> {
  ...
  const result = await runRepresentativeSupplyPromotionWave({
    baseUrl,
    workspaceId,
    apply,
  ...
  if (result.summary.failed > 0 || (apply && result.summary.blocked > 0)) process.exitCode = 1;
}
