...
export function GET(request: Request) {
  try {
    ...
    const targetJurisdictions = targets.map((target) => target.jurisdiction);
    const sourceJurisdictions = sources.flatMap((source) => source.jurisdictions);
    const jurisdictions = [...new Set([...targetJurisdictions, ...sourceJurisdictions])].sort();
