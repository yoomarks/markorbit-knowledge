...
function validateEvaluation(
  ...
  for (const source of evaluation.expectedSources) {
    if (!isContentObjectRefV1(source.content) || source.content.workspaceId !== result.workspaceId) {
...
export function evaluateKnowledgeRetrieval(
  ...
  return {
    ...
    exactChunkHitRate: expectedChunkCount === 0 ? null : exactChunkMatches.size / expectedChunkCount,
