...
const mode = process.env.MARKORBIT_E2E_COMM_PHASE;
if (mode !== "seed" && mode !== "restart-replay") {
  throw new Error("MARKORBIT_E2E_COMM_PHASE must be seed or restart-replay.");
}
...
test(`Managed Communication production bootstrap ${mode}`, async () => {
