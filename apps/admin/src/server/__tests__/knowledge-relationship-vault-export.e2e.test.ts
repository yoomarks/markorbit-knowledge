...
describe("Knowledge relationship production READY-to-Vault acceptance", () => {
  it("uses real governed staging, writes the deterministic Vault path and replays without rewriting", async () => {
    ...
    expect(first.staging.stagingTargetPath).toMatch(/^canonical\/.+\.md$/u);
