...
const LIVE = process.env.MARKORBIT_USPTO_FEE_LIVE_EVIDENCE === "1";
const CANONICAL_URI = "https://www.uspto.gov/trademarks/trademark-fee-information";
const QUERY_TEXT = "base application filing fee Section 1 Section 44 per class";

function required(name: string): string {
  const value = process.env[name]?.trim();
  if (!value) throw new Error(`${name} is required for live USPTO fee evidence`);
  return value;
}
...
liveDescribe("Phase 2 live USPTO official fee research evidence", () => {
  it("replays exact canonical fee evidence and evaluates complete chunk lineage", async () => {
    ...
    const evaluation = runFrozenRetrievalEvaluation(
      {
        schemaVersion: "1.0",
        fixtureId: "retrieval-uspto-official-fee-live",
        fixtureVersion: canonicalHit.document.contentSha256,
        queries: [
          {
            queryId: "uspto-base-application-fee",
            workspaceId,
            queryText: QUERY_TEXT,
            evaluation: {
              k: 10,
      ...
      [{ queryId: "uspto-base-application-fee", result: first }],
    );
    ...
    process.stdout.write(
      `${JSON.stringify(
        {
          event: "phase2.uspto-official-fee.live-evidence.accepted",
          sourceId,
          workspaceId,
