...
export function FoundationalHealthDashboard({ workspaceId }: { workspaceId: string }) {
  ...
  const copy = {
    ...
    currentScope: zh
      ? "当前健康引擎已接入美国与 WIPO；后续新增国家或机构时沿用同一覆盖视图。"
      : "The current health engine covers the United States and WIPO; additional jurisdictions can use the same coverage view.",
  };
                ...
                <tbody className="divide-y divide-slate-100">
                  {gate.targets.map((target) => (
                    <tr key={target.targetId} className="hover:bg-slate-50/70">
                      ...
                      <td className="px-5 py-4 text-xs text-slate-600">{issueLabel(target, zh)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
