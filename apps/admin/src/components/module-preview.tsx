...
const plannedActions: Record<Exclude<ModuleKey, "dashboard">, string[]> = {
  discovery: ["添加主页或批量 Seed", "审核自动发现的候选来源", "维护 Include / Exclude Discovery Policy"],
  sources: ["创建与分类数据源", "按国家、类型和状态筛选", "测试连接、预览采集和检查更新"],
  people: ["查看机构与专业人士候选", "核对公开业务联系方式及来源证据", "把身份解析交给 Core 而不是在此自动合并"],
  knowledge: ["按文档、案例、公告和媒体浏览", "追溯 RawArtifact 与来源证据", "查看等待进入 Core 的 Staging 内容"],
  collection: ["查看正在采集与计划采集工作", "聚合失败任务和需要关注的来源", "从业务视角进入高级 Plan / Run"],
    ...
    <>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
          新后台先回答“现在需要做什么”，再按需下钻到 Plan、Run、Worker、Manifest 等工程对象。未完成的 Runtime 不会伪装成真实采集结果。
        </p>
