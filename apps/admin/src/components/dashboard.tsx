...
const discoveries = [
  {
    title: "USPTO",
    detail: "+13 candidate pages",
    meta: "Official authority · 4 minutes ago",
    tone: "success",
  },
  {
    title: "Thailand DIP",
    detail: "+2 PDF candidates",
    meta: "Official authority · 18 minutes ago",
    tone: "success",
  },
  {
    title: "Professional firm seed batch",
    detail: "+4 professional observations",
    meta: "Public business evidence · 31 minutes ago",
    tone: "warning",
  },
] as const;
  ...
  return (
    <>
      ...
      <section
        ...
        aria-label="Knowledge health preview"
      >
        {metrics.map((metric) => {
          ...
          <div className="divide-y divide-slate-100">
            {discoveries.map((item) => (
              <div key={item.title} className="flex items-start gap-3 px-5 py-4">
                <span
                  className={`mt-0.5 rounded-lg p-2 ${item.tone === "success" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                >
                  {item.tone === "success" ? (
