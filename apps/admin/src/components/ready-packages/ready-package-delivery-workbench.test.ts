...
describe("ReadyPackage delivery workbench Core intake eligibility", () => {
  it("keeps configured VERIFIED delivery actions available", () => {
    ...
    expect(
      isCoreIntakeActionable("VERIFIED", "SUBMISSION_FINALIZATION_PENDING", true),
    ).toBe(true);
  });
  ...
  it("allows a HANDED_OFF package to recover an existing pending submission", () => {
    ...
    expect(
      isCoreIntakeActionable("HANDED_OFF", "SUBMISSION_FINALIZATION_PENDING", true),
    ).toBe(true);
  });
  ...
  it("blocks only HTTP-dependent actions when outbound transport is not configured", () => {
    ...
    expect(
      coreIntakeActionRequiresOutboundTransport("SUBMISSION_FINALIZATION_PENDING"),
    ).toBe(false);
    ...
    expect(
      isCoreIntakeActionable("VERIFIED", "SUBMISSION_FINALIZATION_PENDING", false),
    ).toBe(true);
    expect(
      isCoreIntakeActionable("HANDED_OFF", "SUBMISSION_FINALIZATION_PENDING", false),
    ).toBe(true);
  });
