...
async function readSnapshot(signal?: AbortSignal): Promise<Snapshot> {
  ...
  const response = await fetch(`/api/source-intelligence/reviews/policy-scopes?${params}`, { signal });
...
export function SourceIntelligencePolicyScopes() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "无法读取 D2.14 policy scopes");
    } finally {
      setLoading(false);
    }
  }, [applySnapshot]);
      ...
      <div className="flex flex-col gap-4 border-b border-slate-200 p-5 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-sm leading-6 text-slate-500">
            用人工显式 Cohort 为不同 Source 设置不同 workflow targets。Membership 不从域名、机构、类别或
            Source Value 推断；启用 Cohort 以唯一 Priority 决定优先级，数字越大越优先，Global D2.13 policy
            仅作 fallback。不会自动分组、派单、升级、通知或执行。
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="rounded-xl border border-violet-100 bg-violet-50/60 p-4">
            <div className="flex flex-wrap items-end gap-2">
              <label className="text-xs font-medium text-slate-600">
                Operator label
                <input value={actor} onChange={(event) => setActor(event.target.value)} maxLength={120} className="mt-1 block w-40 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900" />
              </label>
              <label className="text-xs font-medium text-slate-600">
                Cohort 名称
                <input value={name} onChange={(event) => setName(event.target.value)} maxLength={120} className="mt-1 block w-48 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900" />
              </label>
              <label className="text-xs font-medium text-slate-600">
                Priority
                <input value={priority} onChange={(event) => setPriority(event.target.value)} inputMode="numeric" className="mt-1 block w-24 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900" />
              </label>
              <label className="text-xs font-medium text-slate-600">
                领取目标 h
                <input value={claimTarget} onChange={(event) => setClaimTarget(event.target.value)} placeholder="关闭" inputMode="numeric" className="mt-1 block w-24 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900" />
              </label>
              <label className="text-xs font-medium text-slate-600">
                复核目标 h
                <input value={reviewTarget} onChange={(event) => setReviewTarget(event.target.value)} placeholder="关闭" inputMode="numeric" className="mt-1 block w-24 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900" />
              </label>
              ...
              <button type="button" onClick={() => void saveCohort()} disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-violet-700 px-3.5 py-2 text-sm font-semibold text-white disabled:opacity-50">
                <Save size={15} /> {editingCohortId ? "更新 Cohort" : "创建 Cohort"}
              </button>
              {editingCohortId ? (
                <button type="button" onClick={resetEditor} className="rounded-lg border border-slate-300 px-3.5 py-2 text-sm font-medium text-slate-700">取消编辑</button>
              ) : null}
            </div>
            <input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="可选说明：只记录人工定义的用途，不用于自动分类" maxLength={1000} className="mt-3 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900" />
          </div>
          ...
          <div className="rounded-xl border border-slate-200 p-4">
            ...
            <div className="mt-3 flex flex-wrap items-end gap-2">
              <label className="text-xs font-medium text-slate-600">
                Cohort
                <select value={membershipCohortId} onChange={(event) => setMembershipCohortId(event.target.value)} className="mt-1 block w-56 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900">
                  <option value="">选择 Cohort</option>
                  {policyScopes.cohorts.map((cohort) => <option key={cohort.cohortId} value={cohort.cohortId}>{cohort.name} · P{cohort.priority}</option>)}
                </select>
              </label>
              <label className="text-xs font-medium text-slate-600">
                Source
                <select value={membershipSourceId} onChange={(event) => setMembershipSourceId(event.target.value)} className="mt-1 block w-64 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900">
                  <option value="">选择 Source</option>
                  {sources.map((source) => <option key={source.id} value={source.id}>{source.id}</option>)}
                </select>
              </label>
              <button type="button" onClick={() => void changeMembership()} disabled={saving || !membershipCohortId || !membershipSourceId} className="rounded-lg border border-violet-300 bg-violet-50 px-3.5 py-2 text-sm font-semibold text-violet-800 disabled:opacity-50">
          ...
          <div className="grid gap-3 xl:grid-cols-2">
            {policyScopes.cohorts.length === 0 ? (
              <div className="rounded-xl border border-slate-200 p-4 text-sm text-slate-500">尚未创建 Cohort，当前全部 Source 使用 Global fallback。</div>
            ) : policyScopes.cohorts.map((cohort) => {
              const members = policyScopes.memberships.filter((membership) => membership.cohortId === cohort.cohortId);
              return (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${cohort.enabled ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-slate-200 bg-slate-100 text-slate-600"}`}>{cohort.enabled ? "Enabled" : "Disabled"}</span>
                        <span className="rounded-full border border-violet-200 bg-violet-50 px-2 py-0.5 text-xs font-medium text-violet-800">Priority {cohort.priority}</span>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">领取 {formatTarget(cohort.claimTargetHours)} · 复核 {formatTarget(cohort.reviewTargetHours)} · {members.length} Sources</p>
                      {cohort.description ? <p className="mt-2 text-sm text-slate-600">{cohort.description}</p> : null}
                    </div>
                    <button type="button" onClick={() => editCohort(cohort)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700">编辑</button>
                  </div>
                  {members.length > 0 ? <p className="mt-3 break-words text-xs leading-5 text-slate-500">{members.map((membership) => membership.sourceId).join(" · ")}</p> : null}
                </div>
              );
            })}
          </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr><th className="px-4 py-3">Source</th><th className="px-4 py-3">Effective scope</th><th className="px-4 py-3">Targets</th><th className="px-4 py-3">Matched cohorts</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {policyScopes.effectivePolicies.map((policy) => (
                  <tr key={policy.sourceId}>
                    ...
                    <td className="px-4 py-3 text-slate-700">{policy.scope === "COHORT" ? `${policy.cohortName} · P${policy.priority}` : policy.scope === "GLOBAL" ? "Global fallback" : "Unconfigured"}</td>
                    <td className="px-4 py-3 text-slate-600">领取 {formatTarget(policy.claimTargetHours)} · 复核 {formatTarget(policy.reviewTargetHours)}</td>
                    <td className="px-4 py-3 text-xs text-slate-500">{policy.matchedCohortIds.length || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-900">
            Cohort 是人工 workflow metadata，不代表 Source 的真实类别、权威性、法律状态或身份关系。Priority 只用于选择人工 SLA policy；不会成为 Scheduler priority，也不会触发 collection、routing、notification 或 escalation。
          </div>
        </div>
      ) : null}
    </section>
