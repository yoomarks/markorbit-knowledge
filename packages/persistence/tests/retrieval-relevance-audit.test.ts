...
describe("retrieval relevance smoke audit", () => {
  it("configures an explicit deterministic probe for every active foundational target", () => {
    ...
    expect(targets).toHaveLength(74);
    expect(FOUNDATIONAL_RETRIEVAL_RELEVANCE_PROBES).toHaveLength(74);
    expect(new Set(FOUNDATIONAL_RETRIEVAL_RELEVANCE_PROBES.map((probe) => probe.id)).size).toBe(74);
