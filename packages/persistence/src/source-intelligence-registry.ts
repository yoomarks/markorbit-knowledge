...
export class SqliteSourceIntelligenceRepository {
  ...
  save(assessment: SourceIntelligenceAssessment): SourceIntelligenceAssessment {
    if (!isSourceIntelligenceAssessment(assessment)) throw new Error("Invalid SourceIntelligenceAssessment");
  ...
  get(id: string): SourceIntelligenceAssessment | null {
    const row = this.database.prepare("SELECT document_json FROM source_intelligence_assessments WHERE assessment_id = ?").get(id) as { document_json: string } | undefined;
  ...
  getByFingerprint(sourceId: string, inputFingerprint: string): SourceIntelligenceAssessment | null {
    const row = this.database.prepare("SELECT document_json FROM source_intelligence_assessments WHERE source_id = ? AND input_fingerprint = ?").get(sourceId, inputFingerprint) as { document_json: string } | undefined;
    return row ? this.parse(row.document_json) : null;
  }
  ...
  private parse(documentJson: string): SourceIntelligenceAssessment {
    ...
    if (!isSourceIntelligenceAssessment(value)) throw new Error("Persisted SourceIntelligenceAssessment is invalid");
    return value;
  }
}
