...
export function parseUsptoTsdrWebTarget(raw: string): UsptoTsdrWebTarget {
  ...
  if (url.origin !== TSDR_WEB_ORIGIN || url.username || url.password || url.hash) {
    return invalid("TSDR Web target must use the canonical public TSDR origin without credentials/hash");
  }
  ...
  if (status) {
    ...
    return { surface: "STATUS", serialNumber: status[1]!, canonicalUri: url.toString() };
  }
  ...
  if (image) {
    ...
    return { surface: "MARK_IMAGE", serialNumber: image[1]!, canonicalUri: url.toString() };
  }
  ...
  if (url.pathname === "/documentviewer") {
    ...
    return { surface: "DOCUMENT_INDEX", serialNumber, canonicalUri: url.toString() };
  }
