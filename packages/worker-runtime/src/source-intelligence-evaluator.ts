...
function dimension(score: number | null, confidence: "LOW" | "MEDIUM" | "HIGH", ...reasonCodes: string[]): SourceIntelligenceDimension {
...
function authorityScore(level: AuthorityLevel): SourceIntelligenceDimension {
  switch (level) {
    case "PRIMARY": return dimension(100, "HIGH", "EXPLICIT_AUTHORITY_PRIMARY");
    case "SECONDARY": return dimension(80, "HIGH", "EXPLICIT_AUTHORITY_SECONDARY");
    case "PRIVATE": return dimension(65, "HIGH", "EXPLICIT_AUTHORITY_PRIVATE");
    case "COMMENTARY": return dimension(55, "HIGH", "EXPLICIT_AUTHORITY_COMMENTARY");
    case "COMMUNITY": return dimension(35, "HIGH", "EXPLICIT_AUTHORITY_COMMUNITY");
    case "UNKNOWN": return dimension(null, "LOW", "AUTHORITY_NOT_EXPLICITLY_ASSIGNED");
  }
}

function freshnessScore(latestCapturedAt: string | undefined, assessedAt: string): SourceIntelligenceDimension {
...
function evidenceabilityScore(snapshot: SourceIntelligenceInputSnapshot): SourceIntelligenceDimension {
  ...
  return dimension(score, "HIGH", "RAW_ARTIFACT_PROVENANCE_COVERAGE", "IMMUTABLE_ARTIFACT_EVIDENCE_PRESENT");
}
...
function relevanceScore(snapshot: SourceIntelligenceInputSnapshot): SourceIntelligenceDimension {
  if (snapshot.contentNodeCount === 0) {
    return dimension(categoryBaseline(snapshot.sourceCategory), "LOW", "CATEGORY_BASELINE_ONLY", "NO_CONTENT_NODES_YET");
  }
  ...
  return dimension(score, topicalRatio > 0 ? "HIGH" : "MEDIUM", "GRAPH_TOPIC_COVERAGE", ...(snapshot.retainedNodeCount > 0 ? ["HUMAN_RETAINED_EVIDENCE"] : []));
}

function noveltyScore(snapshot: SourceIntelligenceInputSnapshot): SourceIntelligenceDimension {
  if (snapshot.previousGraphNodeCount === undefined || snapshot.previousDistinctArtifactHashCount === undefined) {
  ...
  const artifactDelta = Math.max(0, snapshot.distinctArtifactHashCount - snapshot.previousDistinctArtifactHashCount);
  if (nodeDelta === 0 && artifactDelta === 0) return dimension(10, "HIGH", "NO_NEW_GRAPH_OR_ARTIFACT_EVIDENCE");
  return dimension(Math.min(100, 35 + nodeDelta * 3 + artifactDelta * 12), "HIGH", "NEW_EVIDENCE_SINCE_PREVIOUS_ASSESSMENT");
}

function acquisitionCostScore(snapshot: SourceIntelligenceInputSnapshot): SourceIntelligenceDimension {
  if (snapshot.rawArtifactCount === 0) return dimension(null, "LOW", "NO_ACQUISITION_FOOTPRINT_YET");
  const averageBytes = snapshot.rawArtifactBytes / snapshot.rawArtifactCount;
  const mb = snapshot.rawArtifactBytes / 1_048_576;
  const score = Math.min(100, 10 + Math.log2(Math.max(1, averageBytes / 32_768)) * 10 + Math.log2(Math.max(1, mb)) * 5);
  return dimension(score, "MEDIUM", "OBSERVED_BYTE_FOOTPRINT_PROXY", "COST_SCORE_IS_HEURISTIC_NOT_BILLING_DATA");
}
...
function assessmentId(sourceId: string, inputFingerprint: string): string {
  return `sia_${createHash("sha256").update(`${sourceId}:${inputFingerprint}`).digest("hex").slice(0, 24)}`;
}
...
function scorePriority(dimensions: SourceIntelligenceAssessment["dimensions"]): number {
  ...
  const costPenalty = dimensions.ACQUISITION_COST.score === null ? 0 : dimensions.ACQUISITION_COST.score * 0.1;
...
function rescanFor(tier: SourceIntelligenceTier): SourceIntelligenceAssessment["recommendedRescan"] {
  if (tier === "A") return { mode: "DAYS", intervalDays: 7, reasonCodes: ["TIER_A_WEEKLY_REVIEW"] };
  if (tier === "B") return { mode: "DAYS", intervalDays: 30, reasonCodes: ["TIER_B_MONTHLY_REVIEW"] };
  if (tier === "C") return { mode: "DAYS", intervalDays: 90, reasonCodes: ["TIER_C_QUARTERLY_REVIEW"] };
  return { mode: "MANUAL", reasonCodes: ["TIER_D_MANUAL_ONLY"] };
}

export function evaluateSourceIntelligence(input: SourceIntelligenceEvaluationInput): SourceIntelligenceAssessment {
