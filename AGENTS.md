...
2. Never move MarkOrbit Core information understanding, distillation, knowledge, capability, value-scoring or recommendation logic into this repository. AI-provider execution in Knowledge is evidence acquisition only: preserve exact provenance/raw response evidence, and treat research prompts, instruction meaning, synthesis/distillation and downstream semantic meaning as Brain/Method-owned. Any legacy ADK compatibility surface must follow the frozen AI Evidence Plane owner map. Consumer readiness must project from `CurrentGovernedKnowledgeV1`: `CURRENT != VERIFIED != CONSUMER_ADMISSIBLE != DELIVERED`; ReadyPackage, Brain and Core adapters must never become Knowledge currentness authorities.
