# M12 — WIPO Source Coverage and Operational Supply

## Purpose

M12 extends the source-data plane from the initial USPTO baseline to WIPO without changing the architectural boundary of `markorbit-knowledge`.
...
M12 generalizes the existing foundational-source tooling so both `US` and `WO` use the same controlled path:
